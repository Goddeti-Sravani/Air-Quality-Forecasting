...
def process_features(sample):
    ...
    temperatureHumidityInteration = sample[0][0]*sample[0][1] #temp * Humidity

    #print(len(sample[0][2:7]))
    

    averagePollutants = sum(sample[0][2:7])/5 # Average of all the pollutants

    preprocessedFeatures = np.array([sample[0][4], sample[0][6], sample[0][7], averagePollutants, temperatureHumidityInteration])
#NO2,CO,Proximity,AvgPollutant,TempHumidityInteraction,
    return preprocessedFeatures

 

def make_predictions(inputSample):
    with open('AirQuality.pkl', 'rb') as file:
